...
proctype pex (byte id; byte warp_id; chan pex_u; chan u_pex) {
    ...
    byte local_mem_idx = id + warp_id * PEX_PER_WARP; // The position of a process in the local workgroup memory depends on its unique identifier and block number.
        ...
        :: 	u_pex ? 0, STOP ->
            ...
            :: id == 0 ->
                for (tile_idx : 1..(service.count_elements-1)) { // REDUCE local
                    sum(LOC[warp_id * service.count_elements],LOC[tile_idx + warp_id * service.count_elements]) // access to local memory
                    service.time++;
                }
                ...
                atomic {
                    ...
                    sum(GLOB[0],LOC[warp_id * service.count_elements])
                    service.time = service.time + GMT;
                }
                service.final = true;
            :: else -> skip;
...
proctype warp (byte id; byte u_id; chan u_warp; chan warp_u) {
    ...
    chan pex_warp = [0] of {mtype : Action};
    chan warp_pex = [0] of {byte, mtype : Action};

    xr pex_warp;
    xs warp_pex;

    atomic { 
        for (proc_idx : 0.. PEX_PER_WARP - 1) { 
            run pex(proc_idx, id, pex_warp, warp_pex); 
        } 
    }
        ...
        :: else ->
            ...
            for (proc_idx : 0..(PEX_PER_WARP-1)) {
                atomic{
                    ...
                    if
                    :: done_flag == EOI -> skip;
                    :: else -> warp_pex ! wg_iter, GO;
                    fi
                    proc_count++;
                    if
                    :: proc_count >= PEX_PER_WARP -> 
                        wg_iter++; 
                        proc_count = 0;
                    :: else -> skip;
                    fi
			...
			for (proc_idx : 0..(PEX_PER_WARP - 1)) { pex_warp ? EOI; }
        fi;
        warp_u ! DONE;
    :: u_warp ? 0, STOP ->
        atomic { 
...
proctype unit (byte id; chan dev_u; chan u_dev) {
    chan u_warp = [0] of {byte, mtype : Action};
    chan warp_u = [0] of {mtype : Action};
    ...
    atomic { for (warp_id : 0 .. service.count_warp - 1) { run warp(warp_id, id, u_warp, warp_u); } }
    
    do 
    :: 	dev_u ? wg_id, GO ->
        atomic { 
            for (warp_id : 0 .. service.count_warp - 1) {
                u_warp ! wg_id, GO;
            }
        }
        for (warp_id : 0 .. service.count_warp - 1) {
