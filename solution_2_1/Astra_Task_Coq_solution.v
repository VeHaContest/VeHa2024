...
Definition MAY_EXEC_BIT : Z := 0.
Definition MAY_WRITE_BIT : Z := 1.
Definition MAY_READ_BIT : Z := 2.

(* Определение воспроизводит логику функции исходного кода. *)
Definition inode_permission (tsec_ilev : Z) (isec_ilev : Z) (mask : Z) : Z :=
  match Z.testbit mask MAY_WRITE_BIT with
  | false => 0
  | true => if Z_lt_dec tsec_ilev isec_ilev then -eaccess else 0
  end.
...
(* Используем определения минимума и максимума и связанных с ними функций. *)
Instance ZLattice : Lattice Z := {
  meet := Z.max;
  join := Z.min;
  meetCommutative := Z.max_comm;
  meetAssociative := Z.max_assoc;
  meetAbsorptive  := Z.min_max_absorption;
  meetIdempotent  := Z.max_id;

  joinCommutative := Z.min_comm;
  joinAssociative := Z.min_assoc;
  joinAbsorptive  := Z.max_min_absorption;
  joinIdempotent  := Z.min_id;
}.
...
Section Integrity.
  ...
  Lemma transitionsIntegrity : forall (es : list Event) (st st' : State),
    transitions es st st' -> integrity st -> integrity st'.
  Proof.
    (* Доказательство по индукции на определении переходов. *)
    intros es st st' ts i. induction ts.
    - trivial.
    (* Применяем индуктивную гипотезу, удалёем ненужные факты и разбираем определение перехода. *)
    - apply IHts in i. clear ts IHts. destruct H3.
    (* Раскрываем определения, упрощаем и проводим анализ отдельных случаев. *)
    -- unfold checkRight in H6. unfold integrity, updateAccess, updateSubjectAccess; simpl.
       intros. destruct (eq_dec s s0).
    --- destruct (eq_dec o o0).
    ---- subst s o. assumption.
    ---- unfold integrity in i. apply i in H7. trivial.
    --- unfold integrity in i. apply i in H7. trivial.
    -- unfold integrity, updateAccess, updateSubjectAccess; simpl. intros. destruct (eq_dec s s0).
    --- destruct (eq_dec o o0).
    ---- contradiction.
    ---- subst s. unfold integrity in i. apply i in H6. trivial.
    --- unfold integrity in i. apply i in H6. trivial.
Qed.
End Integrity.
...
(*
  Надо хорошо понимать, как устроено выведение таких простых фактов в Coq.
  В HOL это всё гораздо проще и доказывается автоматически.
*)
Lemma LeqZ : forall a b : Z, ~(a < b)%Z -> (b ≤ a).
Proof.
  intros. admit.
Admitted.

Lemma NLeqZ : forall a b : Z, (a < b)%Z -> ~(b ≤ a).
Proof.
  intros. admit.
Admitted.

(* Тоже из серии познаний, как усложнить то, что в HOL было бы тривиально. *)
Lemma TT : forall X, (true = true -> X) -> X.
Proof.
  intros. admit.
Admitted.
...
Proof.
  (* Раскрываем определения. *)
  intros st mask s o.
  unfold inode_permission, MAY_WRITE_BIT, fromMask, fromMaskb, checkRight.
  (* Разбиваем эквивалентность на пряму и обратную импликацию. *)
  split.
  (* Дальше анализируем случаи сначала по AccessRight, потом по маске. *)
  - intros p a. destruct a.
  -- destruct (Z.testbit mask 2).
  --- trivial.
  --- auto.
  -- destruct (Z.testbit mask 1).
  --- intros. destruct (Z_lt_dec (subjectIntegrity st s) (objectIntegrity st o)).
  ---- unfold eaccess in p. easy.
  ---- apply LeqZ. assumption.
  --- intros. easy.
  -- destruct (Z.testbit mask 0).
  --- trivial.
  --- auto.
  -- intros. easy.
  (* Раскрываем символ подмножества, и рассматриваем, есть в маске бит на запись или нет. *)
  - unfold "⊆". intros f. destruct (Z.testbit mask 1).
  --- destruct (Z_lt_dec (subjectIntegrity st s) (objectIntegrity st o)).
  ---- specialize f with arWrite. simpl in f. apply NLeqZ in l. apply TT in f. easy.
  ---- trivial.
  --- trivial.
Qed.
