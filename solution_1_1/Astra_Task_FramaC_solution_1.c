...
static int compute_mask(struct file *file, unsigned int cmd)
{
    ...
    // В предыдущей строке опечатка: "запрещён"
    ...
    /*@
        ...
        loop invariant isValidMask(mask);
            // Следующий инвариант не исользуется в данной функции и его
            // можно безболезненно удалить. Этот инвариант служит
            // иллюстрацией того, что может потребоваться при верификации
            // более сложного поведения.
            // "Ранее просмотренные события не равны `cmd`".
        loop invariant \forall integer j;
                       0 <= j < i ==> cmd != event_numbers[j][0];
