...
Definition inode_permission (tsec_ilev : Z) (isec_ilev : Z) (mask : Z) : Z :=
  if negb (Z.testbit mask 1)
  then 0
  else
    match Z_le_dec isec_ilev tsec_ilev with
      | left _ => 0
      | _      => eaccess
    end.
...
Section Integrity.
  ...
  Lemma transitionsIntegrity : forall (es : list Event) (st st' : State),
    transitions es st st' -> integrity st -> integrity st'.
  Proof with eauto.
    assert (forall e st st', transition e st st' -> integrity st -> integrity st').
    - induction 1; intros until 2; simpl in *.
      all: unfold updateSubjectAccess, eq_dec in *.
      all: destruct H0, H; try congruence... 
    - induction es; inversion 1...
  Qed.
End Integrity.
...
Proof with try easy; auto using eq_sym.
  unfold inode_permission, fromMask, fromMaskb, "⊆", checkRight in *.
  intros st mask s o.
  assert ((objectIntegrity st o) ≤  (subjectIntegrity st s) <->
         ((objectIntegrity st o) <= (subjectIntegrity st s))%Z) as [].
  { split; unfold "≤"; edestruct Z.min_l_iff... }
  split.
  - intros h1 [] h2...
    destruct mask, Z_le_dec...
    all: rewrite h2 in h1...
  - intros h.
    specialize h with arWrite.
    destruct (Z.testbit mask 1), Z_le_dec as [|h']...
    contradict h'...
Qed.
